...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    let label = if use_list { "affinity list" } else { "affinity mask" };
    let parse = if use_list { parse_cpu_list } else { parse_hex_mask };
    let fmt = if use_list { format_cpu_list } else { format_hex_mask };
    ...
    if pid_mode {
        match positional.as_slice() {
            ...
            [pid_str] => {
                ...
                let pids = if all_tasks { get_task_pids(pid)? } else { vec![pid] };
                for p in pids {
...
fn format_hex_mask(set: &CpuSet) -> String {
    let max_cpu = (0..CpuSet::count())
        .rev()
        .find(|&i| set.is_set(i).unwrap());
...
mod tests {
    ...
    fn test_parse_cpu_list_stride() {
        ...
        for cpu in [0usize, 2, 4, 6, 8, 10] {
            assert!(!set.is_set(cpu).unwrap(), "expected CPU {cpu} to not be set");
        }
    }
